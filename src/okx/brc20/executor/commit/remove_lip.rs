...
impl SwapFunctionData {
  pub(super) fn handle_remove_liq(
    ...
    if params.len() != 6 {
      log::debug!(
        "brc20swap handle_remove_liq: invalid params: {:?}, length: {:?}",
        params.clone(),
        params.len()
      );
    ...
    let token_lp_amt = Brc20Decimal::from_str(&params[2]).map_err(|_e| {
      ExecutionError::ExecutionFailed(BRC20Error::DecimalParseError(params[2].clone()))
    })?;
    let token_0_amt = Brc20Decimal::from_str(&params[3]).map_err(|_e| {
      ExecutionError::ExecutionFailed(BRC20Error::DecimalParseError(params[3].clone()))
    })?;
    let token_1_amt = Brc20Decimal::from_str(&params[4]).map_err(|_e| {
    ...
    let ticker_0_info = match context.load_brc20_ticker_info(&ticker_0)? {
      ...
      None => {
        log::debug!(
          "remove_liq none, ticker_0 not found: {}",
          ticker_0.clone().to_string()
        );
        return Err(ExecutionError::ExecutionFailed(BRC20Error::TickerNotFound(
          ticker_0.clone().to_string(),
    ...
    if token_0_amt.sign() < 0 || token_0_amt.clone().to_u128().0 > ticker_0_info.max_mint_limit {
      log::debug!(
        "remove_liq, token amount bigger than max mint limit: {}, max mint limit: {}",
        token_0_amt.clone(),
    ...
    let ticker_1_info = match context.load_brc20_ticker_info(&ticker_1)? {
      ...
      None => {
        log::debug!(
          "remove_liq none, ticker_1 not found: {}",
          ticker_1.clone().to_string()
        );
        return Err(ExecutionError::ExecutionFailed(BRC20Error::TickerNotFound(
          ticker_1.clone().to_string(),
    ...
    let mut pool_balance =
      match context.load_brc20_module_swap_poolpair_balance(&module_id, &pool_pair)? {
        ...
        None => {
          log::debug!(
            "remove_liq, pool_pair not found: {}",
    ...
    let module_info = match context.load_brc20_module_info(&module_id)? {
      ...
      None => {
        log::debug!(
          "remove_liq none, module_id not found: {}",
          module_id.clone()
        );
        return Err(ExecutionError::ExecutionFailed(
      ...
      == Ordering::Greater
    {
      ...
      if lp_fee.cmp(&Brc20Decimal::from_u128(0, 0).unwrap()) == Ordering::Greater {
        ...
        // lp_fee update
        let lp_fee_key = BRC20ModuleAddressLPTokenBalanceKey::new(module_id, &sender, &pool_pair);
    ...
    {
      log::debug!(
        "remove_liq over slippage, sender: {:?}, token0: {:?}, expect: {:?}",
        sender.clone(),
        amt_0.clone(),
        token_0_amt.clone(),
      );
    ...
    {
      log::debug!(
        "remove_liq over slippage, sender: {:?}, token1: {:?}, expect: {:?}",
        sender.clone(),
        amt_1.clone(),
        token_1_amt.clone(),
      );
    ...
    if pool_balance.lp_balance.cmp(&token_lp_amt.clone()) == Ordering::Less {
      log::debug!(
        "remove_liq insufficient lp token balance, sender: {:?}, pool_lp: {:?}, token_lp: {:?}",
        sender.clone(),
    ...
    if pool_balance.tick_balance[token_0_idx].cmp(&amt_0.clone()) == Ordering::Less {
      log::debug!(
        "remove_liq insufficient pool balance, sender: {:?}, pool: {:?}",
        sender.clone(),
        pool_pair.clone(),
      );
      return Err(ExecutionError::ExecutionFailed(
    ...
    context.update_brc20_module_address_token_balance(&sender, module_id, &ticker_0, token_0_balance)?;
    context.update_brc20_module_address_token_balance(&sender, module_id, &ticker_1, token_1_balance)?;
