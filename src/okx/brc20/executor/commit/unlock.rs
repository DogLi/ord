...
impl SwapFunctionData {
  pub(super) fn handle_unlock(
    ...
    log::info!("brc20swap handle_unlock: {:?}", self);
    ...
    let sender: UtxoAddress =
      UtxoAddress::from_str(self.address.as_ref().unwrap(), chain.network()).map_err(|e| {
        log::debug!(
          "brc20swap error handle_unlock: address parse error: {}, {}",
          e.to_string(),
    ...
    match context.load_brc20_module_swap_poolpair_balance(&module_id, &pool_pair)? {
      ...
      None => {
        log::debug!(
          "brc20swap error handle_unlock: pool_pair {} not exists",
    ...
    let mut lp_balance = match context.load_brc20_module_address_lp_token_balance(&lp_key)? {
      ...
      None => {
        log::debug!(
          "brc20swap error handle_unlock: lp_key {:?} not exists",
          lp_key.clone(),
        );
        return Err(ExecutionError::ExecutionFailed(
    ...
    if lp_balance.locked_balance.cmp(&token_amt_decimal) == Ordering::Less {
      log::debug!(
        "brc20swap error handle_unlock: insufficient lp token balance: has: {:?}, wants: {:?}",
        lp_balance.locked_balance.clone(),
        token_amt_decimal.clone()
      );
      return Err(ExecutionError::ExecutionFailed(
        BRC20Error::InsufficientLPTokenBalance(lp_balance.locked_balance, token_amt_decimal),
      ));
    }
    ...
    log::debug!("brc20swap handle_unlock finished: {:?}", self);
    Ok(())
  }
}
