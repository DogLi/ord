...
impl SwapFunctionData {
  pub(super) fn handle_swap(
    ...
    if params.len() != 7 {
      log::debug!(
        "brc20swap handle_swap: invalid params: {:?}, length: {:?}",
        params,
        params.len()
      );
    ...
    if direction != DIRECTION_EXACT_IN && direction != DIRECTION_EXACT_OUT {
      log::error!("brc20swap handle_swap: invalid direction: {:?}", direction);
      return Err(ExecutionError::ExecutionFailed(
    ...
    let token_in_amt: Brc20Decimal;
    let token_out: BRC20Ticker;
    let token_out_amt: Brc20Decimal;
    if direction == DIRECTION_EXACT_IN {
      token_in = BRC20Ticker::from_str(&params[2])
        .map_err(|e| ExecutionError::ExecutionFailed(BRC20Error::TickerParse(e)))?;
      token_in_amt = Brc20Decimal::from_str(&params[3]).map_err(|_e| {
      ...
      token_in_amt = Brc20Decimal::from_str(&params[5]).map_err(|_e| {
        log::debug!("swap, decimal parse error: {}", params[5].clone());
        ExecutionError::ExecutionFailed(BRC20Error::DecimalParseError(params[5].clone()))
      })?;
    }
    ...
    let token_in_info = match context.load_brc20_ticker_info(&token_in)? {
      ...
      None => {
        log::debug!("swap, token_in not found: {}", token_in.clone().to_string());
        return Err(ExecutionError::ExecutionFailed(BRC20Error::TickerNotFound(
          token_in.to_string(),
        )));
      }
    };
    if token_in_amt.sign() < 0 || token_in_amt.clone().to_u128().0 > token_in_info.max_mint_limit {
      log::debug!(
        "swap, token amount bigger than max mint limit: {}, max mint limit: {}",
        token_in_amt.clone(),
        token_in_info.max_mint_limit.clone()
      );
      return Err(ExecutionError::ExecutionFailed(
    ...
    if token_out_amt.sign() < 0 || token_out_amt.clone().to_u128().0 > token_out_info.max_mint_limit
    {
      log::debug!(
        "swap, token amount bigger than max mint limit: {}, max mint limit: {}",
        token_out_amt,
        token_out_info.max_mint_limit
      );
      return Err(ExecutionError::ExecutionFailed(
    ...
    let module_info = match context.load_brc20_module_info(&module_id)? {
      ...
      None => {
        log::debug!("swap, module_id not found: {}", module_id.clone());
        return Err(ExecutionError::ExecutionFailed(
    ...
    if pool_pair.starts_with(&token_in.clone()) {
      token_in_idx = 0;
      token_out_idx = 1;
    } else {
      token_in_idx = 1;
    ...
    let slippage_amt = Brc20Decimal::from_str(&params[6]).map_err(|_e| {
      log::debug!("swap, decimal slippage parse error: {}", params[6].clone());
    ...
    if direction == DIRECTION_EXACT_IN {
      if module_info.fee_rate_swap > Brc20Decimal::from_u128(0, 0).unwrap() {
        // with fee
      ...
      if amount_out.clone() < amount_out_min.clone() {
        log::debug!(
          "brc20swap handle_swap: amount_out < amount_out_min: {:?}, {:?}",
          amount_out.clone(),
        ...
        amount_in = pool_balance.tick_balance[token_in_idx]
          ...
          .div(
            pool_balance.tick_balance[token_out_idx]
              ...
              .add(Brc20Decimal::from_u128(1, token_in_amt.clone().get_precition()).unwrap()),
          )
      } else {
        amount_in = pool_balance.tick_balance[token_in_idx]
          .clone()
      ...
      if amount_in_max.clone() < amount_in.clone() {
        log::debug!(
          "brc20swap handle_swap: amount_in_max < amount_in: {:?}, {:?}",
          amount_in_max.clone(),
          amount_in.clone()
        );
        return Err(ExecutionError::ExecutionFailed(BRC20Error::SlippageError()));
    ...
    if pool_balance.tick_balance[token_out_idx].clone() < amount_out.clone() {
      log::debug!(
        "brc20swap handle_swap: pool token out balance insufficient: has: {:?}, wants: {:?}",
    ...
    if token_in_balance.swap_account_balance < Brc20Decimal::from_u128(0, 0).unwrap() {
      log::debug!(
        "brc20swap handle_swap: user token in balance insufficient: has: {:?}, wants: {:?}",
