...
impl SwapFunctionData {
  pub(super) fn handle_deploy_pool(
    ...
    let token0_amt = match check_ticker_verify(context, &token0, &String::new()) {
      ...
      Err(_) => {
        log::debug!(
          "brc20swap handle_deploy_pool: token0 {:?} not found",
    ...
    let token1_amt = match check_ticker_verify(context, &token1, &String::new()) {
      ...
      Err(_) => {
        log::debug!(
          "brc20swap handle_deploy_pool: token1 {:?} not found",
    ...
    {
      log::debug!(
        "brc20 swap::execute: pool_pair {}/{} already exists, inscription_id = {}",
        token0.clone().to_string(),
        token1.clone().to_string(),
        module_id.clone()
      );
    ...
    context.update_brc20_module_swap_poolpair_balance(
      &module_id,
      &pool_pair,
      BRC20ModuleSwapPoolPairBalance::new_with_balance(token0_amt, token1_amt),
    )?;
